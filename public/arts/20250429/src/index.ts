...
const getRatioFromCurrentTime = (clockType: CLOCK_TYPE) => {
  const date = new Date();
  let degree = 0;
  switch (clockType) {
    case CLOCK_TYPE.HOURS:
      degree = ((date.getHours() % 12) + date.getMinutes() / 60) / 12;
      break;
    case CLOCK_TYPE.MINUTES:
      degree = (date.getMinutes() + date.getSeconds() / 60) / 60;
      break;
    case CLOCK_TYPE.SECONDS:
      degree = (date.getSeconds() + date.getMilliseconds() / 1000) / 60;
      break;
    case CLOCK_TYPE.MILLISECONDS:
      degree = date.getMilliseconds() / 1000;
      break;
  }
  return degree;
};
...
const render = () => {
  ...
  uniforms.hours.value = getRatioFromCurrentTime(CLOCK_TYPE.HOURS);
  uniforms.minutes.value = getRatioFromCurrentTime(CLOCK_TYPE.MINUTES);
  uniforms.seconds.value = getRatioFromCurrentTime(CLOCK_TYPE.SECONDS);
  uniforms.milliseconds.value = easing.easeOutExpo(
    getRatioFromCurrentTime(CLOCK_TYPE.MILLISECONDS)
  );
