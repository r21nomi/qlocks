...
void main() {
  ...
  float _e = ease(milliseconds);
  float ee = floor(50.0 * seconds) * _e;
  float iii = (10.0 + battery * 100.0) * (_e - 1.0);
  uv = floor(uv * iii) / iii;

  float shapeH =(0.15 * ee) * _e;
  vec2 shapeUv = uv;
  vec2 _uv = shapeUv;
  float n = snoise(vec2(uv.x * 1.0, ee)) * (0.05 * ee);
  float si = sin((uv.x * 1.5) + time * 2.0) * 1.0 * (0.5 * _e);
  float shape = 1.0 - step(1.0, rect(vec2(_uv.x, _uv.y + n + si), ratio, vec2(1.0, shapeH)));

  // colors
  vec3 bgColor = vec3(0.025);
  vec3 shapeColor = vec3(0.55);
  shapeColor = vec3(0.0, 0.75, 0.1);
  shapeColor = vec3(0.85, 0.1, 0.0);
  shapeColor = mix(vec3(0.85, 0.1, 0.0), vec3(0.0, 0.75, 0.1), battery);
